...
class TaskAssignmentsController < ApplicationController
  ...
  def new
    @all_taks = Task.includes(:task_items).all
  ...
  def create
    params.dig(:task_assignment, :task_item_ids)
        .reject(&:empty?)
        .each do |item_id|
      @patient.task_assignments.new(task_assignment_params.merge({task_item_id: item_id.to_i}))
    end

    if @patient.save
      return redirect_to patients_path,
                         notice: t('.index.messages.sucess_create')
    end
  ...
  def update
    if @task_assignment.update(task_assignment_params)
      return redirect_to patient_task_assignments_path(@patient),
                         notice: t('.index.messages.sucess_update')
    end
